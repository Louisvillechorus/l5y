...
assets='<script id="l5yassets">window.L5Y_SFX=%s;window.L5Y_PAPER=%s;window.L5Y_IMG=%s;</script>'%(json.dumps(sfx),json.dumps(paper),json.dumps(img))
...
print('embedded sounds:',len(sfx),'paper scans:',len(paper),'images:',len(img))
