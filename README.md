...
**▮ The house rig — 75″ landscape TV, audience up to 25 ft.** The start screen asks WHO IS ON TONIGHT (Maegan & Landon / Andrew & Charlie) — pick the couple first; only the photos/videos change. The projection opens on the preshow card (title + clock); the first GO takes it out. The stage is composed in three zones: the character's name on the left (their color, plus HER PHONE / HIS LAPTOP), the device in the center, and the era on the right — month and year in gold, which rolls through the months (forward or backward) on every era change. Close-ups render in a wide reading window that always shows full text width, cropping only to the register's reading region; the MacBook takes the whole screen under a top info bar; non-diegetic beats drop everything (full-bleed). Setup: drag the projection window onto the TV, press F. (Portrait/vertical support remains in the engine — a portrait display auto-fills with the classic floating labels, and **R** on the projection window rotates the feed 0°/90°/270° if a rig ever needs it.)
